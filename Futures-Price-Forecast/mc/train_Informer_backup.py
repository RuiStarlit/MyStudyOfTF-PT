...
class Train_Informer():
    ...
    def preview_noam(self):
        if self.noam is False:
            raise RuntimeError('Only support Noam Schedule now')
        noam = self.scheduler
        ...
        if not train_all:
            ...
            for f in self.train_f:
                ...
                if self.val_every is True:
                    val_loss, val_r2, val_rate = self.val(True)
                    print(f'After training {f.split("/")[-1].split(".")[0]}  Val_Loss:{val_loss:.6f} '
                          f'|R2:{val_r2:.6f} |Rate:{val_rate:.3f} |'
    ...
    def test(self, ic_name):
        ...
        with torch.no_grad():
            for i in range(len(dataset)):
                ...
                rate[i] = frate(pred[:, -1, 0], Y[:, -1, 0]).detach().cpu().numpy()
                pred_list.append(pred[:, -1, 0].detach().cpu().numpy())
                y_list.append(Y[:, -1, 0].detach().cpu().numpy())
            pred = np.concatenate(pred_list)
            y = np.concatenate(y_list)
...
class MyDataset_p():
    def __init__(self, file_name, batch_size, pred_len=11, enc_seq_len=20, label_len=10, initpoint=1000):
        self.name = file_name
        self.__read_data__()
        self.batch_size = batch_size
