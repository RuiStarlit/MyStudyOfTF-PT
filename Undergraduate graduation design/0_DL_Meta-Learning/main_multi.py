...
class TrainNet():
    ...
    def train(self, epochs=None, ckp=0):
        ...
        for epoch in bar:
            ...
            # scheduler
            if (epoch+ckp) in self.args.milestone:
                self.lr = self.lr * 0.1
            ...
            for iteration, (inputs, labels) in enumerate(train_dataloader):
                ...
                if (iteration+1) % self.args.interval == 0:
                    ...
                    if self.args.model == 'resnet32':
                        pseudo_model = ResNet32(self.num_classes, option=self.resnet_option).to(self.device)
                        # self.mdoel = torchvision.models.resnet34(
                        #     True, {'num_classes':self.num_classes}
                        #     ).to(self.device)
                    elif self.args.model == 'resnet101':
                        pseudo_model = torchvision.models.resnet.resnet101(
                            {'num_classes':self.num_classes}).to(self.device)
                    elif self.args.model == 'resnet34':
                        pseudo_model = torchvision.models.resnet34(
                            {'num_classes':self.num_classes}).to(self.device)
                    elif self.args.model == 'wrn28':
                        pseudo_model = WideResNet(28, self.num_classes, 10, nc=3).to(self.device)
                    else:
                        raise NotImplementedError()
                    # pseudo_model = WideResNet(28, self.num_classes, 10, nc=3).to(self.device)
                ...
                if self.args.threshold > 0 and self.args.burn_in <= epoch+ckp:
                    ...
                    if self.args.semi == 'mixlabel':
                        ...
                        if c_ind.sum() >=  n_ind.sum()/5 and n_ind.sum() > 0:
                            ...
                            semi_loss = F.mse_loss(outputs[n_ind], new_y)
                            loss = torch.mean(weight * loss_reshpae) + self.args.semi_p*semi_loss
            ...
            if epoch == (self.args.burn_in -1):
                self.save(self.name+'e80', verbose=True, acc=test_acc, epoch=epoch+1+ckp)
            if test_acc > best_acc:
                best_acc = test_acc
                self.save(self.name+f'-best', verbose=False, acc=test_acc, epoch=epoch+1+ckp)
    ...
    def record_weight(self, weight, y, loss, size=0, reset=False):
        if (self.rweight is None) or reset:
            self.rweight = np.zeros((size, 3))
        self.rweight[self.rindex:self.rindex+y.shape[0]] = np.concatenate(
            [weight, y, loss], axis=1
        )
        self.rindex += y.shape[0]
        if self.rindex == self.record_weight.shape[0]:
            print('[Debug] Need to reset rweight')

    def plot_weight(self, rweight):
        # 不能与torch一起使用，会报错，需要单独绘图
        weight, y, loss = rweight[:, 0], rweight[:, 1], rweight[:, 2]
        ax = plt.subplot(projection='3d')
        ax.scatter(loss, y, weight)
        ax.set_zlabel('Weight')
        ax.set_xlabel('Loss')
        ax.set_ylabel('Label')
        plt.show()
    
    def show_weight(self, epoch_p):
        train_dataloader, meta_dataloader, test_dataloader, imbalanced_num_list = cifar_dataloader(
            seed=self.args.seed,
            dataset=self.args.dataset,
            num_meta_total=self.args.num_meta,
        ...
        batch_size = self.args.batch_size
        rweight = np.zeros((len(train_dataloader.dataset), 3))
        self.model.train()
        with torch.no_grad():
            for i, (inputs, labels) in enumerate(train_dataloader):
                ...
                loss = F.cross_entropy(outputs, labels.long(), reduction='none') # default:mean
                loss_reshpae = loss.reshape((-1, 1))
                weight = self.meta_net(loss_reshpae, labels, self.args.loss_p, epoch_p)
                weight_n = weight.detach().cpu().numpy()
                labels_n = labels.detach().cpu().numpy()
                loss_n = loss_reshpae.detach().cpu().numpy()
                rweight[i*batch_size:(i+1)*batch_size, :] = np.concatenate(
                    [weight_n, labels_n[:, None], loss_n], axis=1
                )
        return rweight
