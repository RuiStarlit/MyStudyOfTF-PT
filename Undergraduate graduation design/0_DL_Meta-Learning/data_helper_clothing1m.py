...
def prepare_data(batch_size, num_workers=0):
    num_classes = 14
    ...
    train_gold_loader = torch.utils.data.DataLoader(train_data_gold,   batch_size=batch_size, shuffle=(gold_sampler is None),
                                                                 num_workers=num_workers, pin_memory=True, sampler=gold_sampler)
    train_silver_loader = torch.utils.data.DataLoader(train_data_silver, batch_size=batch_size, shuffle=(silver_sampler is None),
                                                      num_workers=num_workers, pin_memory=True, sampler=silver_sampler)
