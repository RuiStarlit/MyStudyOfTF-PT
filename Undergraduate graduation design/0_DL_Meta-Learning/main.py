        ...
        for epoch in bar:
            ...
            for iteration, (inputs, labels) in enumerate(train_dataloader):
                ...
                # if (iteration+1) % self.args.interval == 0:
                    ...
                    pseudo_model = ResNet32(self.num_classes).to(device)
                    ...
                    for inner in range(innerepochs):
                        ...
                        meta_weight = self.meta_net(pseudo_loss.data) # Tensor.data do not contain grad
                ...
                with torch.no_grad():
                    weight = self.meta_net(loss)
                loss = torch.mean(weight * loss)
                if self.args.threshold > 0:
                    ind = torch.ge(weight,self.args.threshold)[:,0]
