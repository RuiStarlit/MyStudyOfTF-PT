    ...
    def __call__(self, i):
        ...
        if i == 0:
            ...
            for j in range(self.label_len):
                Y1 = np.hstack((temp[-1 - j - self.shift - self.batch_size: -1 - j - self.shift], Y1))
        else:
            r_index = self.indexes[i * self.batch_size - self.label_len - self.shift: (i + 1) * self.batch_size]
            temp = self.y[r_index]
            Y1 = self.y[batch_index]
            for j in range(self.label_len):
                Y1 = np.hstack((temp[-1 - j - self.shift - y_len:-1 - j - self.shift], Y1))
        # 向未来取趋势时间序列
        if i >= int(np.ceil(self.n / self.batch_size)):
            ...
            for j in range(self.pred_len):
                Y2 = np.hstack((Y2, temp[1 + j: 1 + j + y_len + 1]))
...
class MyDataset_p():
    ...
    def __call__(self, i):
        ...
        if i == 0:
            ...
            # temp = np.zeros((self.label_len + self.shift, 1))
            Y1 = self.y[batch_index]
            nY1 = self.ny[batch_index]
            # temp = np.concatenate((temp, Y1))
            temp = self.y[0: self.batch_size]
            ntemp = self.ny[0: self.batch_size]
            for i in range(self.label_len):
                Y1 = np.hstack((temp[-1 - i - self.shift - self.batch_size: -1 - i - self.shift], Y1))
                nY1 = np.hstack((ntemp[-1 - i - self.shift - self.batch_size: -1 - i - self.shift], nY1))
        else:
            r_index = self.indexes[i * self.batch_size - self.label_len - self.shift: (i + 1) * self.batch_size]
            temp = self.y[r_index]
            ntemp = self.ny[r_index]
            Y1 = self.y[batch_index]
            nY1 = self.ny[batch_index]
            for j in range(self.label_len):
                Y1 = np.hstack((temp[-1 - j - self.shift - y_len:-1 - j - self.shift], Y1))
                nY1 = np.hstack((ntemp[-1 - j - self.shift - y_len:-1 - j - self.shift], nY1))
        # 向未来取趋势时间序列
        if i >= int(self.n / self.batch_size):
            # temp = np.full((self.pred_len, 1), self.y[-1, 0])
            temp = self.y[batch_index]
            ntemp = self.ny[batch_index]
            batch_index = batch_index[:-self.pred_len - 1]
            Y2 = self.y[batch_index]
            nY2 = self.ny[batch_index]
            # temp = np.concatenate((Y2, temp))
            for j in range(self.pred_len):
                Y2 = np.hstack((Y2, temp[1 + j: 1 + j + y_len + 1]))
                nY2 = np.hstack((nY2, ntemp[1 + j: 1 + j + y_len + 1]))
            ...
            for j in range(self.pred_len):
                Y2 = np.hstack((Y2, temp[1 + j: 1 + j + y_len]))
                nY2 = np.hstack((nY2, ntemp[1 + j: 1 + j + y_len]))
        Y = np.hstack((Y1[:, :-1], Y2))
        nY = np.hstack((nY1[:, :-1], nY2))

        X = self.x[batch_index, -self.enc_seq_len:, :]
        Y = Y[:, :, np.newaxis]
        nY = nY[:, :, np.newaxis]
        Y = np.concatenate((Y, nY), axis=2)
        X = torch.from_numpy(X).to(self.device).float()
        Y = torch.from_numpy(Y)
        return X, Y
    ...
    def __del__(self):
        del self.x, self.y, self.indexes, self.ny
...
class ValDataset_p():
    def __init__(self, file_name, pred_len=3, enc_seq_len=20, label_len=1, device='cuda'):
        self.name = file_name
        self.__read_data__()
        self.n = self.y.shape[0]
        self.enc_seq_len = enc_seq_len
    ...
    def __call__(self):
        ...
        # 向过去取历史时间序列
        # temp = np.zeros((self.label_len + self.shift, 1))
        Y1 = self.y[0+self.shift + self.label_len: -self.pred_len - 1]
        nY1 = self.ny[0 + self.shift + self.label_len: -self.pred_len - 1]
        temp = self.y[0:-self.pred_len - 1]
        ntemp = self.ny[0:-self.pred_len - 1]
        # temp = np.concatenate((temp, Y1))
        for i in range(self.label_len):
            Y1 = np.hstack((temp[-1 - self.shift - y_len: -1 - i - self.shift], Y1))
            nY1 = np.hstack((ntemp[-1 - self.shift - y_len: -1 - i - self.shift], nY1))

        # 向未来取趋势时间序列
        # temp = np.full((self.pred_len, 1), self.y[-1, 0])
        Y2 = self.y[0+self.shift + self.label_len: -self.pred_len - 1]
        nY2 = self.ny[0 + self.shift + self.label_len: -self.pred_len - 1]
        temp = self.y[0+self.shift + self.label_len:]
        ntemp = self.ny[0 + self.shift + self.label_len:]
        # temp = np.concatenate((Y2, temp))
        for i in range(self.pred_len):
            Y2 = np.hstack((Y2, temp[1 + i: 1 + i + y_len]))
            nY2 = np.hstack((nY2, ntemp[1 + i: 1 + i + y_len]))

        Y = np.hstack((Y1[:, :-1], Y2))
        nY = np.hstack((Y1[:, :-1], nY2))
